...
export default function InfoPdam(props) {
    ...
    const onDeleteHandler = () => {
        router.delete(route("admin.delete-info-pdam", model.id), {
            onSuccess: () => {
    ...
    const reload = useCallback(
        debounce((query) => {
            router.get(route("admin.info-pdam"), query, {
                preserveScroll: true,
                preserveState: true,
            });
        }, 150),
    ...
    return (
        <div className="py-6 px-8">
            <Modal
                ...
                title={model ? "Update Info PDAM" : "Tambah Info PDAM"}
            >
                <Form
                    model={model}
                    setModel={setModel}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                {info.length > 0 ? (
                    ...
                    <p className="text-center py-16 w-full col-span-4">
                        Belum ada info di yang tambahkan
                    </p>
                )}
            </div>
        </div>
...
InfoPdam.layout = (page) => (
    <AdminLayout children={page} title={"Kelola Info PDAM"} />
);
