...
import ShowTagihan from "./ShowTagihan";
...
export default function ShowPemasanganBaru({ id }) {
    ...
    const deleteHandler = (id) => {
        router.get(route("pelanggan.delete-pembayaran-tagihan", id));
    };
    console.log(data);
    return (
        <div>
                    ...
                    <div className="bg-red-500 py-2 px-4 rounded-md my-2">
                        <div>
                            <p className="text-white text-xs">
                                Ups status meteran anda belum di proses silahkan
                                melakukan pembayaran terlebih dahulu pada
                                halaman berikut{" "}
                                <Link
                                    href={route(
                                        "pelanggan.lihat-tagihan-pemasangan-baru",
                                        id
                                    )}
            ...
            <div
                className={`${menu == "informasi tagihan air" ? "" : "hidden"}`}
            >
                <ShowTagihan id={data && data[0].id} />
            </div>
        </div>
    );
}
